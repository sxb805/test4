...
async function loginAndGetContext() {
  ...
  if (json?.result !== 0 || !json?.data) throw new Error(`login failed: ${json?.msg || res.statusText}`);
  let accessToken = '';
  try {
...
async function fillAddFormAndSubmit(page, code) {
  ...
  if ((await inputs.count()) < 2) {
    console.log('DEBUG_ADD:no-enough-inputs', await inputs.count());
    return false;
  }
  ...
  if (!(await treeSelect.isVisible().catch(() => false))) {
    console.log('DEBUG_ADD:tree-select-not-visible');
    return false;
  }
  await treeSelect.click();
  await page.waitForTimeout(300);

  // Try search first, then fallback to first visible tree node.
  ...
  if (!(await treeNode.isVisible().catch(() => false))) {
    console.log('DEBUG_ADD:tree-node-not-visible');
    await page.keyboard.press('Escape').catch(() => {});
    return false;
  }
  ...
  if (!submitOk) {
    console.log('DEBUG_ADD:submit-button-not-clickable');
    return false;
  }
  ...
  if (!ok) {
    ...
    console.log('DEBUG_ADD:no-success-tip', txt || 'empty-tip');
  }
  return ok;
}
...
(async () => {
  ...
  try {
    const c = await loginAndGetContext();
    ...
    if ((await rowForAction.count()) === 0) {
      ...
      console.log('DEBUG_EDIT:no-row-for-action');
    }
    ...
    if (await editBtn.isVisible().catch(()=>false)) {
      ...
      if (!result.edit) {
        const errTip = page.locator('.ant-message .ant-message-notice:visible').first();
        const txt = await errTip.textContent().catch(() => '');
        console.log('DEBUG_EDIT:no-success-tip', txt || 'empty-tip');
      }
      await page.waitForTimeout(800);
      await cleanupOverlays(page);
    } else {
      console.log('DEBUG_EDIT:edit-button-not-visible');
    }
    ...
    if (await delBtn.isVisible().catch(()=>false)) {
      ...
      if (await pop.isVisible().catch(()=>false)) {
        ...
        if (!result.delete) {
          const errTip = page.locator('.ant-message .ant-message-notice:visible').first();
          const txt = await errTip.textContent().catch(() => '');
          console.log('DEBUG_DELETE:no-success-tip', txt || 'empty-tip');
        }
      }
    } else {
      console.log('DEBUG_DELETE:delete-button-not-visible');
    }
  } catch (e) {
