  ...
  effects: {
    page: [
      function* ({ payload = {} }, { call }) {
        ...
        const res = yield call(workPlanService.page, {
          ...formData,
    ...
    *view({ payload = {} }, { call }) {
      const res = yield call(workPlanService.view, { id: payload.id });
      if (res?.result === 0) {
        return res?.data || {};
      }
      return {};
    ...
    *saveOrUpdate({ payload = {} }, { call }) {
      ...
      const service = type === "save" ? workPlanService.save : workPlanService.update;
      const res = yield call(service, { ...params, id });
      return { success: res?.result === 0, msg: res?.msg || "" };
    },
    *deleteItems({ payload = {} }, { call }) {
      const { ids = [] } = payload;
      const res = yield call(workPlanService.delete, { ids: ids.join(",") });
      return { success: res?.result === 0, msg: res?.msg || "" };
    },
  },
  reducers: {
